...
-- %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
--
-- Total = 25 + 0 + 15 + 10 = 50/100
--
-- %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
module HW3_Martinez_graded where
...
uniquemain (Program fds)
    | (countMain fds) == 1 = Left (Program fds)
    | (countMain fds) == 0 = Right "No Main Procedure"
    | (countMain fds) >= 2 = Right "Too many main procedures"
    | otherwise            = Right "some other error..."


-- uniquemain helper functions:
-- get name of a function FunDecl
funcName :: FunDecl -> Name
funcName (FunDecl _ n _ _ _) = n

-- get list of Name for list of FunDecl
funcNames :: [FunDecl] -> [Name]
funcNames [] = []
funcNames (x:xs) = funcName x : funcNames xs  

-- get list of 'main' functions in list of FunDecl
getMainList :: [FunDecl] -> [Name]
getMainList l = filter (=="main") (funcNames l)

-- get number of 'main' in list of FunDecl
countMain :: [FunDecl] -> Int
countMain l = length (getMainList l)
...
main  = FunDecl INT "main" [] [] (Return (FunCall "fac" [ICon 5]))
func  = FunDecl INT "func" [] [] (Return (ICon 3))
foo   = FunDecl INT "foo"  [Decl "x" INT, Decl "y" INT] [Decl "a" INT] (Return (ICon 5))
fds0 = [func,foo]
fds1 = [func,foo,main]
fds2 = [func,foo,main,main]
ex3 = Program fds2
ex4 = Program fds0
expA = Var "z"
expB = BinOp EQ (Var "i") (ICon 0)
expC = (FunCall "fac" [BinOp Sub (Var "i") (ICon 1)])
cmd = Assign "x" (ICon 1)
facExp  = (BinOp EQ (Var "i") (ICon 0))
facCmd1 = (Return (ICon 1))
facCmd2 = (Return (BinOp Times (Var "i") (FunCall "fac" [BinOp Sub (Var "i") (ICon 1)]))) 
fac2 = FunDecl INT "fac" [Decl "i" INT] [] (IfThenElse facExp facCmd1 facCmd2)
-}
...
--
-- Problem 3. 15/25. 
-- This isn't quite right (-10); it will give false positives.
--
-- %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
noundefined (Program fds) = subseteq (usedvarslist fds, defvarslist fds)

--
-- I would suggest that you define the following helper functions.
--

defvarslist :: [FunDecl] -> [Name]
defvarslist []     = []
defvarslist (f:fs) = defvars f ++ defvarslist fs

usedvarslist :: [FunDecl] -> [Name]
usedvarslist []     = []
usedvarslist (f:fs) = usedvars f ++ usedvarslist fs
...
defvars (FunDecl _ _ params locals _) = map declName (params++locals)
...
varsCmd :: Cmd -> [Name]
varsCmd (Assign n e)         = n : varsExp e
varsCmd (While e c)          = varsExp e ++ varsCmd c
varsCmd (Seq c1 c2)          = varsCmd c1 ++ varsCmd c2
varsCmd (IfThen e c)         = varsExp e ++ varsCmd c
varsCmd (IfThenElse e c1 c2) = varsExp e ++ varsCmd c1 ++ varsCmd c2
varsCmd (Return e)           = varsExp e
varsCmd (Skip)               = []

-- This computes all the names used in an Exp. Note that it is interdefined with varsCmd:
varsExp :: Exp -> [Name]
varsExp (Var n)         = n : []
varsExp (UnaryOp _ e)   = varsExp e
varsExp (BinOp _ e1 e2) = varsExp e1 ++ varsExp e2
varsExp (FunCall n _)   = [] -- varsExp (FunCall n _)   = n : [] -- if funcs are vars?
varsExp (ICon _)        = []
varsExp (FCon _)        = []
varsExp (BCon _)        = []
...
{-
-- check results
progdefined :: Program -> [Name]
progdefined (Program fds) = defvarslist fds

progused :: Program -> [Name]
progused (Program fds) = usedvarslist fds
-}
...
alwaysReturn (Program fds) 
    | (length noreturns) == 0 = Left (Program fds)
    | (length noreturns) >= 1 = Right ("Following commands don't return: " ++ show noreturns)
    where noreturns = noreturnlist fds
